...
curl -s -X GET "https://api.cloudflare.com/client/v4/<<REDACTED-CLOUDFLARE-TOKEN>>/cfd_tunnel/3ae754bf-6be0-4eb8-82d9-7d9f543ef9b2" \
  -H "Authorization: Bearer <<REDACTED-CLOUDFLARE-TOKEN>>" | jq '.result'
```
...
**Create New Tunnel:**
```bash
curl -s -X POST "https://api.cloudflare.com/client/v4/<<REDACTED-CLOUDFLARE-TOKEN>>/cfd_tunnel" \
  -H "Authorization: Bearer <<REDACTED-CLOUDFLARE-TOKEN>>" \
...
**List Tunnels:**
```bash
curl -s -X GET "https://api.cloudflare.com/client/v4/<<REDACTED-CLOUDFLARE-TOKEN>>/cfd_tunnel" \
  -H "Authorization: Bearer <<REDACTED-CLOUDFLARE-TOKEN>>"
```
